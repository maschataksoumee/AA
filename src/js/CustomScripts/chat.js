...
var localProjectId = queryProjectId;

$(document).ready(function(){
	$("#btnQuery").addClass("active");
	var UserId = "UserId="+localUserId;
	var data_all = UserId + "&ProjectCode=" + localProjectId;
	// alert(data_all);
	$.ajax
	({
		type: "POST",
	...
	$("#sendquery").click(function(){
		...
		data_all = UserId + "&" + txtAuthorQuery + "&ProjectCode=" + localProjectId;
		console.log(data_all);
		$.ajax
		({
			type: "POST",
			url: urlConstant + "Chats/NewQuery",
			dataType: "json",
			data: data_all,
			success: function (response)
			{
				...
				if(response.IsOk == true)
				{
					...
					window.location.href="testQuery.php?queryProjectId="+localProjectId;
				}
				else
				{
					swal
